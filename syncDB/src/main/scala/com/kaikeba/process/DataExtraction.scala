package com.kaikeba.process

import java.util
import java.util.Properties

import com.kaikeba.sync.utils.HbaseUtils
...
object DataExtraction {
  ...
  def main(args: Array[String]): Unit = {
    ...
    properties.setProperty("group.id",groupId)
     val kafkaConsumer = new FlinkKafkaConsumer[String](topicName,new SimpleStringSchema(),properties)
    ...
    val binlogStream: DataStream[CanalLog] = sourceStream.map(line => {
      ...
      // String data=logfileName+"#CS#" + logfileOffset + "#CS#" + dbName + "#CS#" + tableName  + "#CS#" + eventType + "#CS#" + list1 + initCount ;
    ...
    binlogStream.map(line =>{
       //获取要操作的列
      //line : CanalLog(mysql-bin.000002,666,kaikeba,commodity,INSERT,[[commodityId, 10, true], [commodityName, MacBook Pro, true], [commodityTypeId, 3, true], [originalPrice, 43000.0, true], [activityPrice, 40000.0, true]],1)
         ...
         if(eventType.equals("DELETE")){
             //由于购物车表比较特殊这里对表进行划分 ，由于没有特定的主键标识，需要设计对应的rowkey。
              tableName match{
                ...
                case _  =>{
                    //拼接hbase表的主键
                   val rowkey=line.dbName+"_"+line.tableName+"_"+getPrimaryKey(fields)
                   //数据库同步操作
                    //删除表数据
                   deleteHbase(tableName,rowkey,columnFamily)
                }
  ...
  /**
    *获取购物车分析的rowkey =添加时间+用户id+产品id+上家id
    * @param fields
    * @return
    */
  ...
  /**
    ...
    * @param rowkey
    */
  def deleteHbase(tableName:String,rowkey:String,columnFamily:String): Unit ={

    HbaseUtils.deleteDataByRowkey(TableName.valueOf(tableName),rowkey:String,columnFamily:String)
  }
  ...
  /**
    ...
    */
  def insertUpdateHbase(tableName:String,rowkey:String,columnFamily:String,triggerColumns: util.ArrayList[UpdateFields]): Unit ={
  ...
  //获取变更的列
   def getTriggerColumns(columns:String,eventType:String): util.ArrayList[UpdateFields] ={
       //将字符串转换成数组
        ...
        val arrayList = new util.ArrayList[UpdateFields]()
