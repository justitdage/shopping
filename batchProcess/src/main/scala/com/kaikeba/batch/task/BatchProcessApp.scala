...
import com.kaikeba.batch.bean.{MerchantCountMoney, OrderRecord, OrderRecordWide}
import com.kaikeba.batch.util.{HBaseTableInputFormat, HbaseUtils}
...
object BatchProcessApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val reducedDataSet: DataSet[MerchantCountMoney] = groupByData.reduceGroup(iter => {
      iter.reduce((x, y) => {
        MerchantCountMoney(x.merchantId, x.orderNum + y.orderNum, x.payAmount + y.orderNum, x.time)
      })
    })
