package com.kaikeba.task

import com.kaikeba.`trait`.DataProcess
import com.kaikeba.bean.{ChannelUserFreshness, _}
  ...
  override def process(dataStream: DataStream[Message]): Unit = {
   //1、将实时解析出的数据转换成新鲜度实体类
    ...
    val result: DataStream[ChannelUserFreshness] = timeWindowStream.reduce((c1, c2) => {
      ...
      val dateFiled: String = c1.dateFiled
    ...
    result.addSink(c =>{
      ...
      val dateFiled: String = c.dateFiled
